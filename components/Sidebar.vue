...
<template>
  <div class="flex flex-col h-full text-sm border-r border-gray-300/80 select-none" :style="{ width: '350px' }">
    <NarBar />

    <div class="flex-1 px-4 py-3 overflow-y-auto">
      <details v-for="({ name, version, used }) in Object.values(store.libs)" :key="name" class="my-0.5">
        <summary
          class="flex w-full items-center justify-between cursor-pointer gap-1 hover:bg-gray-100 rounded pl-1 py-px pr-3"
        ...
        <div class="pl-5">
          <div
            v-for="item in used"
            :key="item"
            class="cursor-pointer hover:bg-gray-100 rounded px-3 py-px my-0.5"
