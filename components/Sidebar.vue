  ...
  <div class="flex flex-col h-full border-r border-gray-300/80" :style="{ width: '350px' }">
    ...
    <div class="flex-1 px-4 py-3 overflow-y-auto">
      <details v-for="({ name, version, used }) in Object.values(store.libs)" :key="name">
        <summary class="flex w-full justify-between cursor-pointer" @click="!used.length && store.selectLib(name)">
        ...
        <div class="pl-5">
          <div v-for="item in used" :key="item" class="cursor-pointer" @click="store.selectLib(name, item)">
            {{ item }}
          </div>
        </div>
      </details>
